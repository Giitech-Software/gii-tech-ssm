...
export default function TeacherExamGradesPage() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Exam Grade Entry</h1>
        <p className="mt-1 text-sm text-slate-600">Record subject marks for configured class examinations.</p>
      </div>
      ...
      <section className="grid gap-3 border bg-white p-4 md:grid-cols-[1fr_1fr_auto]">
        ...
        <button disabled={!selectedExam || !subject.trim() || loading} onClick={loadExistingMarks} className="rounded-md border px-3 py-2 text-sm font-medium text-primary disabled:opacity-50">Load Marks</button>
      </section>
      <section className="overflow-x-auto border bg-white">
        <div className="flex items-center justify-between gap-3 border-b px-4 py-3">
          ...
          <button disabled={!selectedExam || !subject.trim() || loading || !students.length} onClick={saveMarks} className="flex items-center gap-2 rounded-md bg-primary px-3 py-2 text-sm font-medium text-white disabled:opacity-50"><Save size={16} /> Save Marks</button>
        </div>
        <table className="min-w-full text-left text-sm">
          <thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="px-4 py-3">Student</th><th className="w-32 px-4 py-3">Class score</th><th className="w-32 px-4 py-3">Exam score</th><th className="w-32 px-4 py-3">Final score</th></tr></thead>
          <tbody className="divide-y">
            {students.map((student) => <tr key={student.studentId}><td className="px-4 py-3"><p className="font-medium">{student.displayName}</p><p className="text-xs text-slate-500">{student.studentId}</p></td><td className="px-4 py-3"><input type="number" min="0" max="100" value={classScores[student.studentId] || ""} onChange={(event) => setClassScores({ ...classScores, [student.studentId]: event.target.value })} className="w-full rounded-md border px-3 py-2" /></td><td className="px-4 py-3"><input type="number" min="0" max="100" value={examScores[student.studentId] || ""} onChange={(event) => setExamScores({ ...examScores, [student.studentId]: event.target.value })} className="w-full rounded-md border px-3 py-2" /></td><td className="px-4 py-3"><input type="number" min="0" max="100" value={marks[student.studentId] || ""} onChange={(event) => setMarks({ ...marks, [student.studentId]: event.target.value })} className="w-full rounded-md border px-3 py-2" /></td></tr>)}
            {!loading && !students.length && <tr><td colSpan={4} className="px-4 py-10 text-center text-slate-500">Select an exam assessment to load its class list.</td></tr>}
          </tbody>
        </table>
      </section>
    </div>
