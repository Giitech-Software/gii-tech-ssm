  ...
  return (
    <div className="space-y-5">
      ...
      <section className="grid gap-3 border bg-white p-4 md:grid-cols-[1fr_1fr_auto]">
        ...
        <input value={subject} onChange={(event) => { setSubject(event.target.value); setMarks({}); }} placeholder="Subject" className="rounded-md border px-3 py-2" />
      ...
      <section className="overflow-x-auto border bg-white">
        ...
        <table className="min-w-full text-left text-sm">
          <thead className="bg-gray-50 text-xs uppercase text-gray-500"><tr><th className="px-4 py-3">Student</th><th className="w-40 px-4 py-3">Mark</th></tr></thead>
          <tbody className="divide-y">
            {students.map((student) => <tr key={student.studentId}><td className="px-4 py-3"><p className="font-medium">{student.displayName}</p><p className="text-xs text-gray-500">{student.studentId}</p></td><td className="px-4 py-3"><input type="number" min="0" max="100" value={marks[student.studentId] || ""} onChange={(event) => setMarks({ ...marks, [student.studentId]: event.target.value })} className="w-full rounded-md border px-3 py-2" /></td></tr>)}
            {!loading && !students.length && <tr><td colSpan={2} className="px-4 py-10 text-center text-gray-500">Select an exam assessment to load its class list.</td></tr>}
          </tbody>
        </table>
      </section>
    </div>
