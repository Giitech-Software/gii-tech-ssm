    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
        <section className="space-y-3">
          ...
          <div className="overflow-x-auto border bg-white"><table className="min-w-full text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="w-12 px-4 py-3"><button title="Select all" onClick={toggleAll}>{sourceStudents.length > 0 && selectedIds.length === sourceStudents.length ? <CheckSquare size={17} /> : <Square size={17} />}</button></th><th className="px-4 py-3">Student</th><th className="px-4 py-3">Current Placement</th><th className="px-4 py-3">Academic Year</th></tr></thead><tbody className="divide-y">{sourceStudents.map((student) => <tr key={student.id} onClick={() => toggleStudent(student.id)} className="cursor-pointer hover:bg-primary"><td className="px-4 py-3">{selectedIds.includes(student.id) ? <CheckSquare className="text-primary" size={17} /> : <Square className="text-slate-400" size={17} />}</td><td className="px-4 py-3"><p className="font-medium">{student.displayName}</p><p className="text-xs text-slate-500">{student.studentId}</p></td><td className="px-4 py-3">{className(student.classId)}<p className="text-xs text-slate-500">{streamName(student.streamId || student.stream)}</p></td><td className="px-4 py-3 text-slate-600">{student.academicYear || "Not set"}</td></tr>)}{!loading && !sourceStudents.length && <tr><td colSpan={4} className="px-4 py-10 text-center text-slate-500">Choose a source class to preview active students.</td></tr>}</tbody></table></div>
        </section>
        <aside className="space-y-3 border bg-white p-4"><h2 className="font-semibold">Target Placement</h2><Select label="Target class" value={targetClassId} onChange={(value) => { setTargetClassId(value); setTargetStreamId(""); }} options={options.classes.map((item) => ({ value: item.classId || item.id, label: item.name }))} /><Select label="Target stream" value={targetStreamId} onChange={setTargetStreamId} options={targetStreams.map((item) => ({ value: item.streamId || item.id, label: item.name }))} optional /><label className="block space-y-1 text-sm font-medium text-slate-700"><span>Academic year</span><input required value={academicYear} onChange={(event) => setAcademicYear(event.target.value)} className="w-full rounded-md border px-3 py-2" /></label><div className="border-t pt-3 text-sm text-slate-600"><p>Selected: <strong>{selectedStudents.length}</strong></p><p className="mt-1">Destination: <strong>{targetClassId ? className(targetClassId) : "-"}</strong></p></div><button disabled={saving || !selectedStudents.length || !targetClassId} onClick={executePromotion} className="flex w-full items-center justify-center gap-2 rounded-md bg-primary px-3 py-2 text-sm font-medium text-white disabled:opacity-50"><ArrowRight size={16} />{saving ? "Promoting..." : "Promote Selected"}</button></aside>
      </div>
      <section className="border bg-white"><h2 className="flex items-center gap-2 border-b px-4 py-3 font-semibold"><History size={17} /> Recent Promotion History</h2><div className="overflow-x-auto"><table className="min-w-full text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="px-4 py-3">Timestamp</th><th className="px-4 py-3">Student</th><th className="px-4 py-3">From</th><th className="px-4 py-3">To</th><th className="px-4 py-3">Batch</th></tr></thead><tbody className="divide-y">{history.slice(0, 100).map((item) => <tr key={item.id}><td className="whitespace-nowrap px-4 py-3 text-slate-500">{formatDate(item.createdAt)}</td><td className="px-4 py-3"><p className="font-medium">{item.studentName}</p><p className="text-xs text-slate-500">{item.studentId}</p></td><td className="px-4 py-3">{className(item.fromClassId)}<p className="text-xs text-slate-500">{streamName(item.fromStreamId)}</p></td><td className="px-4 py-3">{className(item.toClassId)}<p className="text-xs text-slate-500">{streamName(item.toStreamId)} | {item.toAcademicYear}</p></td><td className="px-4 py-3 text-xs text-slate-500">{item.batchId}</td></tr>)}{!history.length && <tr><td colSpan={5} className="px-4 py-8 text-center text-slate-500">No promotion history yet.</td></tr>}</tbody></table></div></section>
    </div>
  );
}
