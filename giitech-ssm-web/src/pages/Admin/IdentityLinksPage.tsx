...
export default function IdentityLinksPage() {
  ...
  return (
    <div className="space-y-5">
      <div><h1 className="text-2xl font-bold text-gray-900">Parent-Student Links</h1><p className="mt-1 text-sm text-gray-600">Resolve family access by maintaining both parent and student profiles together.</p></div>
      ...
      <section className="grid gap-3 border bg-white p-4 md:grid-cols-[1fr_1fr_auto]">
        ...
        <button disabled={!studentId || !parentId || saving} onClick={link} className="flex items-center justify-center gap-2 rounded-md bg-indigo-600 px-3 py-2 text-sm font-medium text-white disabled:opacity-50"><Link2 size={16} /> Link Profiles</button>
      </section>
      ...
      <label className="relative block max-w-md"><Search className="absolute left-3 top-2.5 text-gray-400" size={17} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search student name or ID" className="w-full rounded-md border py-2 pl-9 pr-3 text-sm" /></label>
      <section className="overflow-x-auto border bg-white">
        <table className="min-w-full text-left text-sm"><thead className="bg-gray-50 text-xs uppercase text-gray-500"><tr><th className="px-4 py-3">Student</th><th className="px-4 py-3">Parent</th><th className="px-4 py-3">Link Status</th><th className="px-4 py-3 text-right">Action</th></tr></thead><tbody className="divide-y">{filteredStudents.map((student) => { const parent = student.parentId ? parentsById.get(student.parentId) : undefined; const synced = isSynced(student, parent); return <tr key={student.id}><td className="px-4 py-3"><p className="font-medium">{student.displayName}</p><p className="text-xs text-gray-500">{student.studentId || student.id}</p></td><td className="px-4 py-3">{parent ? <><p>{parent.displayName}</p><p className="text-xs text-gray-500">{parent.parentId}</p></> : <span className="text-gray-500">Unlinked</span>}</td><td className="px-4 py-3"><span className={`rounded-full px-2 py-1 text-xs font-medium ${synced ? "bg-green-50 text-green-700" : "bg-amber-50 text-amber-700"}`}>{synced ? "linked" : parent ? "needs sync" : "needs review"}</span></td><td className="px-4 py-3 text-right">{parent && <button title="Remove parent link" disabled={saving} onClick={() => unlink(student)} className="rounded-md p-2 text-red-600 hover:bg-red-50 disabled:opacity-50"><Unlink size={16} /></button>}</td></tr>; })}{!filteredStudents.length && <tr><td colSpan={4} className="px-4 py-10 text-center text-gray-500"><Users className="mx-auto mb-2" size={20} />No students found.</td></tr>}</tbody></table>
      </section>
    </div>
  );
}
...
function Summary({ title, value, detail }: { title: string; value: number; detail: string }) {
  return <div className="border bg-white p-4"><p className="text-xs uppercase text-gray-500">{title}</p><p className="mt-1 text-2xl font-bold text-gray-900">{value}</p><p className="mt-1 text-xs text-gray-500">{detail}</p></div>;
}
