...
export default function CommunicationsPage() {
  ...
  return (
    <div className="space-y-5">
      <div><h1 className="text-2xl font-bold text-gray-900">Communications</h1><p className="mt-1 text-sm text-gray-600">Publish school notices and maintain the academic calendar.</p></div>
      ...
      {tab === "announcements" ? (
        ...
        <div className="grid gap-6 xl:grid-cols-[360px_1fr]">
          ...
          <div className="space-y-3">{loading ? <Loading /> : events.map((item) => <article key={item.id} className="border bg-white p-4"><div className="flex items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><h2 className="font-semibold text-gray-900">{item.title}</h2><Badge>{item.eventType}</Badge><Badge>{item.audience}</Badge></div><p className="mt-2 text-sm text-gray-600">{item.startDate}{item.endDate ? ` - ${item.endDate}` : ""}{item.location ? ` | ${item.location}` : ""}</p><p className="mt-2 text-sm text-gray-700">{item.description}</p></div><RowActions onEdit={() => { setEditingEvent(item.id); setEventForm({ title: item.title, description: item.description, eventType: item.eventType, audience: item.audience, startDate: item.startDate, endDate: item.endDate, location: item.location, createdBy: item.createdBy }); }} onDelete={() => removeEvent(item)} /></div></article>)}</div>
        </div>
      )}
    </div>
  );
...
function Field({ label, children }: { label: string; children: React.ReactNode }) { return <label className="block space-y-1 text-sm font-medium text-gray-700"><span>{label}</span>{children}</label>; }
...
function Badge({ children }: { children: React.ReactNode }) { return <span className="rounded-full bg-gray-100 px-2 py-0.5 text-xs capitalize text-gray-600">{children}</span>; }
function Loading() { return <p className="border bg-white py-10 text-center text-sm text-gray-500">Loading...</p>; }
function Tab({ active, onClick, icon, label }: { active: boolean; onClick: () => void; icon: React.ReactNode; label: string }) { return <button type="button" onClick={onClick} className={`flex items-center gap-2 border-b-2 px-4 py-3 text-sm font-medium ${active ? "border-indigo-600 text-indigo-700" : "border-transparent text-gray-500 hover:text-gray-800"}`}>{icon}{label}</button>; }
function RowActions({ onEdit, onDelete }: { onEdit: () => void; onDelete: () => void }) { return <div className="flex gap-1"><button title="Edit" onClick={onEdit} className="rounded-md p-2 text-indigo-600 hover:bg-indigo-50"><Pencil size={16} /></button><button title="Delete" onClick={onDelete} className="rounded-md p-2 text-red-600 hover:bg-red-50"><Trash2 size={16} /></button></div>; }
function FormActions({ saving, editing, onCancel }: { saving: boolean; editing: boolean; onCancel: () => void }) { return <div className="flex gap-2"><button disabled={saving} className="flex items-center gap-2 rounded-md bg-indigo-600 px-3 py-2 text-sm font-medium text-white disabled:opacity-60">{editing ? <Send size={15} /> : <Plus size={15} />}{saving ? "Saving..." : editing ? "Save Changes" : "Create"}</button>{editing && <button type="button" onClick={onCancel} className="flex items-center gap-1 rounded-md border px-3 py-2 text-sm"><X size={15} />Cancel</button>}</div>; }
