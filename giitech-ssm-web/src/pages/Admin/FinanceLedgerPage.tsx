...
export default function FinanceLedgerPage() {
  ...
  return (
    <div className="space-y-5">
      <div><h1 className="text-2xl font-bold text-slate-900">Student Fee Ledger</h1><p className="mt-1 text-sm text-slate-600">Track student charges, payments, balances, and receipts.</p></div>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_420px]">
        <div className="space-y-3">
          <label className="relative block max-w-md"><Search className="absolute left-3 top-2.5 text-slate-400" size={17} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search student name or ID" className="w-full rounded-md border py-2 pl-9 pr-3 text-sm" /></label>
          <div className="overflow-x-auto border bg-white"><table className="min-w-full text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="px-4 py-3">Student</th><th className="px-4 py-3">Charged</th><th className="px-4 py-3">Paid</th><th className="px-4 py-3">Balance</th></tr></thead><tbody className="divide-y">{!loading && filteredSummaries.map((item) => <tr key={item.studentId} onClick={() => setSelectedStudentId(item.studentId)} className={`cursor-pointer hover:bg-primary ${selectedStudentId === item.studentId ? "bg-primary" : ""}`}><td className="px-4 py-3"><p className="font-medium">{item.studentName}</p><p className="text-xs text-slate-500">{item.studentId}</p></td><td className="px-4 py-3">{currency(item.charged)}</td><td className="px-4 py-3 text-green-700">{currency(item.paid)}</td><td className="px-4 py-3 font-medium text-red-700">{currency(item.balance)}</td></tr>)}{loading && <tr><td colSpan={4} className="px-4 py-10 text-center text-slate-500">Loading ledgers...</td></tr>}{!loading && !filteredSummaries.length && <tr><td colSpan={4} className="px-4 py-10 text-center text-slate-500">No ledger records yet. Select a student to add the first charge.</td></tr>}</tbody></table></div>
      ...
      <form onSubmit={assignBulkCharges} className="grid gap-3 border bg-white p-4 md:grid-cols-[minmax(0,1fr)_160px_130px_160px_auto]">
        <div className="md:col-span-5"><h2 className="flex items-center gap-2 font-semibold"><Users size={17} /> Assign Fee Structure to Class</h2><p className="mt-1 text-xs text-slate-500">Creates charges for matching active students and skips existing charges for the same period.</p></div>
        ...
        <button disabled={saving} className="rounded-md bg-primary px-3 py-2 text-sm font-medium text-white disabled:opacity-60">Assign Charges</button>
      </form>
      {selectedStudent && <div className="grid gap-5 xl:grid-cols-2"><History title="Charges">{selectedCharges.map((item) => <HistoryRow key={item.id} title={item.description} meta={`${item.term} | ${item.academicYear}`} amount={currency(item.amount)} />)}</History><History title="Payments">{selectedPayments.map((item) => <HistoryRow key={item.id} title={item.receiptNumber} meta={`${item.paymentMethod} | ${item.term}`} amount={currency(item.amount)} action={<button title="Download receipt" onClick={() => printReceipt(item)} className="rounded-md p-2 text-primary hover:bg-primary"><Printer size={16} /></button>} />)}</History></div>}
    </div>
  );
}

function Summary({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) { return <div className="border bg-white p-4"><div className="flex items-center gap-2 text-slate-500">{icon}<span className="text-xs uppercase">{label}</span></div><p className="mt-2 text-xl font-bold text-slate-900">{value}</p></div>; }
function EntryForm({ title, onSubmit, saving, children }: { title: string; onSubmit: (event: React.FormEvent) => void; saving: boolean; children: React.ReactNode }) { return <form onSubmit={onSubmit} className="grid gap-3 border bg-white p-4"><h3 className="font-semibold">{title}</h3>{children}<button disabled={saving} className="flex items-center justify-center gap-2 rounded-md bg-primary px-3 py-2 text-sm font-medium text-white disabled:opacity-60"><Plus size={15} />{saving ? "Saving..." : title}</button></form>; }
...
function History({ title, children }: { title: string; children: React.ReactNode }) { return <section className="border bg-white"><h2 className="border-b px-4 py-3 font-semibold">{title}</h2><div className="divide-y">{children || <p className="px-4 py-6 text-sm text-slate-500">No records yet.</p>}</div></section>; }
function HistoryRow({ title, meta, amount, action }: { title: string; meta: string; amount: string; action?: React.ReactNode }) { return <div className="flex items-center justify-between gap-3 px-4 py-3"><div><p className="text-sm font-medium">{title}</p><p className="text-xs text-slate-500">{meta}</p></div><div className="flex items-center gap-2"><p className="text-sm font-semibold">{amount}</p>{action}</div></div>; }
