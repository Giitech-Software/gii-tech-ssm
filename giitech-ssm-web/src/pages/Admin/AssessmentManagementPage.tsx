  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-600">Configure academic terms and class exam windows.</p>
      </div>
      ...
      <section className="grid gap-5 xl:grid-cols-[340px_1fr]">
        <form onSubmit={saveTerm} className="space-y-3 border bg-white p-4">
          ...
          <button disabled={saving} className="flex w-full items-center justify-center gap-2 rounded-md bg-indigo-600 px-3 py-2 text-sm font-medium text-white disabled:opacity-50"><Plus size={16} /> Add Term</button>
        </form>
        <div className="overflow-x-auto border bg-white">
          <table className="min-w-full text-left text-sm"><thead className="bg-gray-50 text-xs uppercase text-gray-500"><tr><th className="px-4 py-3">Term</th><th className="px-4 py-3">Dates</th><th className="px-4 py-3">Status</th><th className="px-4 py-3 text-right">Action</th></tr></thead><tbody className="divide-y">{terms.map((term) => <tr key={term.id}><td className="px-4 py-3"><p className="font-medium">{term.name}</p><p className="text-xs text-gray-500">{term.academicYear}</p></td><td className="px-4 py-3">{term.startDate} to {term.endDate}</td><td className="px-4 py-3 capitalize">{term.status}</td><td className="px-4 py-3 text-right"><button title="Delete term" onClick={async () => { await deleteTerm(term.id); await loadData(); }} className="rounded-md p-2 text-red-600 hover:bg-red-50"><Trash2 size={16} /></button></td></tr>)}</tbody></table>
        </div>
      </section>
      ...
      <section className="grid gap-5 xl:grid-cols-[340px_1fr]">
        <form onSubmit={saveExam} className="space-y-3 border bg-white p-4">
          ...
          <button disabled={saving || !terms.length} className="flex w-full items-center justify-center gap-2 rounded-md bg-indigo-600 px-3 py-2 text-sm font-medium text-white disabled:opacity-50"><Plus size={16} /> Add Exam</button>
        </form>
        <div className="overflow-x-auto border bg-white">
          <table className="min-w-full text-left text-sm"><thead className="bg-gray-50 text-xs uppercase text-gray-500"><tr><th className="px-4 py-3">Exam</th><th className="px-4 py-3">Class</th><th className="px-4 py-3">Term</th><th className="px-4 py-3 text-right">Action</th></tr></thead><tbody className="divide-y">{exams.map((exam) => <tr key={exam.id}><td className="px-4 py-3"><p className="font-medium">{exam.name}</p><p className="text-xs text-gray-500">{exam.startDate}{exam.endDate ? ` to ${exam.endDate}` : ""}</p></td><td className="px-4 py-3">{classNames.get(exam.classId) || exam.classId}</td><td className="px-4 py-3">{exam.term}<p className="text-xs text-gray-500">{exam.academicYear}</p></td><td className="px-4 py-3 text-right"><button title="Delete exam" onClick={async () => { await deleteExamAssessment(exam.id); await loadData(); }} className="rounded-md p-2 text-red-600 hover:bg-red-50"><Trash2 size={16} /></button></td></tr>)}</tbody></table>
        </div>
      </section>
    </div>
  );
